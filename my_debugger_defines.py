...
#   Structures for CreateProcessA() function
    ...
    #
    #   https://docs.microsoft.com/en-us/windows/win32/api/processthreadsapi/ns-processthreadsapi-startupinfoa?redirectedfrom=MSDN
...
class STARTUPINFO(Structure):
    ...
    # Contains information about a newly created process and
    # its primary thread.
...
class PROCESS_INFORMATION(Structure):
    _fields_ = [
        ('hProcess',        HANDLE),    #   process in all functions that perform operations on the process object.
        ('hThread',         HANDLE),    #   handle to the primary thread of the newly created process. specify the thread in all functions that perform operations on the thread object.
        ('dwProcessId',     DWORD),     #   value is valid from the time the process is created until all handles to the process are closed and the process object is freed; at this point, the identifier may be reused.
        ('dwThreadId',      DWORD),     #   value is valid from the time the thread is created until all handles to the thread are closed and the thread object is freed; at this point, the identifier may be reused.
    ]


    #   References:     WINDOWS 10 create process           https://docs.microsoft.com/en-us/windows/win32/procthread/creating-processes


#   @TODO   https://blog.golang.org/cgo
